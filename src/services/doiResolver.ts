...
export class DoiResolver {
   ...
   * 将 DOI 解析为期刊和论文元数据
   */
  static async resolveDoi(doi: string): Promise<DoiMetadata> {
    const cleanedDoi = doi.trim();
    if (!cleanedDoi) return {};
  ...
  private static async fetchFromOpenAlex(doi: string): Promise<DoiMetadata> {
    ...
    if (authorships.length > 0) {
      const first = authorships.find((a: any) => a.author_position === 'first') || authorships[0];
      const last = authorships.find((a: any) => a.author_position === 'last') || authorships[authorships.length - 1];
