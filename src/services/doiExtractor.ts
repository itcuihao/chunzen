...
export class DoiExtractor {
  // DOI 正则：10.xxxx/xxx
  private static DOI_REGEX = /\b(10\.\d{4,9}\/[^\s"<>{}|\\^`\[\]]+)/gi;
  // ISSN 正则：XXXX-XXXX
  private static ISSN_REGEX = /\b(\d{4}-\d{3}[\dX])\b/gi;

  static extractDoi(text: string): string | undefined {
  ...
  static extractIssn(text: string): string[] {
   ...
   */
  static extractJournalName(text: string): string | undefined {
   ...
   */
  static extract(text: string): { doi?: string; issns: string[]; journal?: string } {
    return {
