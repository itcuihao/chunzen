...
let lastHoveredSentenceId = null;
...
async function loadPdfDocument() {
    try {
        ...
        (0, selection_1.setupSelectionHandler)(textLayer, vscode);
    }
    catch (err) {
...
async function renderCurrentPage() {
    ...
    const { sentences: newSentences, spanToSentence: newSpanMap } = (0, textLayer_1.buildTextLayer)(textLayer, items, viewport);
    sentences.clear();
...
function bindTextLayerEvents() {
    textLayer.addEventListener('mouseover', onSpanMouseover);
    textLayer.addEventListener('mouseout', onSpanMouseout);
    textLayer.addEventListener('click', onSpanClick);
}
function onSpanMouseover(e) {
    const span = e.target;
    if (!span.dataset.sentenceId)
        return;
    const sid = span.dataset.sentenceId;
    if (sid === lastHoveredSentenceId)
        return;
    clearHover();
...
function onSpanMouseout(e) {
    const span = e.target;
    const sid = span.dataset.sentenceId;
    if (!sid)
        return;
    const relatedSid = e.relatedTarget?.dataset?.sentenceId;
    if (relatedSid === sid)
        return;
