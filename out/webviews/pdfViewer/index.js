...
const textLayer_1 = require("./textLayer");
...
let currentRichContent = null;
const pageTranslationsCache = new Map();
const pageRichContentCache = new Map();
const readPages = new Set();
const repeatCandidateSignaturesByPage = new Map();
const repeatSignaturePages = new Map();
let paragraphHoverOverlay = null;
// DOM refs
...
const hudPageInput = document.getElementById('hud-page-input');
...
async function loadPdfDocument() {
    try {
        ...
        (0, pdfRenderer_1.initPdfJs)(window.PDFJS_WORKER);
        pdfDoc = await (0, pdfRenderer_1.loadPdf)(window.PDF_SRC);
        ...
        try {
            ...
            if (meta?.info?.Title)
                pdfTitleEl.textContent = meta.info.Title;
        }
        catch { /* ignore */ }
...
async function renderCurrentPage() {
    ...
    const viewport = await (0, pdfRenderer_1.renderPageToCanvas)(page, canvas, scale);
    ...
    const { paragraphs: newParagraphs, columnsCount } = (0, textLayer_1.buildTextLayer)(textLayer, renderedRichContent, viewport, {
...
function normalizeRepeatSignature(text) {
    return text
        .toLowerCase()
...
function isRepeatNoiseCandidate(para, viewportWidth, viewportHeight) {
    if (para.lineMarker === 'horizontal-rule')
        return false;
    if (!para.text || !para.text.trim())
        return false;
    const text = para.text.trim();
    if (text.length < 14 || text.length > 260)
        return false;
    ...
    const hasDoiCue = /\bdoi:\s*/i.test(text) ||
        /\bdoi\.org\b/i.test(lower) ||
        /\b10\.\d{4,9}\//.test(text);
    const hasMetaCue = hasDoiCue ||
...
function addRepeatSignature(pageNumber, signature) {
...
function removePageRepeatSignatures(pageNumber) {
    ...
    for (const sig of prev) {
        ...
        if (pages.size === 0)
            repeatSignaturePages.delete(sig);
    }
}
function applyRepeatedNoiseSkips(pageNumber, paragraphs, viewportWidth, viewportHeight) {
    removePageRepeatSignatures(pageNumber);
    const signatures = new Set();
    for (const para of paragraphs) {
        if (!isRepeatNoiseCandidate(para, viewportWidth, viewportHeight))
            continue;
        const sig = normalizeRepeatSignature(para.text);
        if (!sig || sig.length < 12)
            continue;
        signatures.add(sig);
    }
    repeatCandidateSignaturesByPage.set(pageNumber, signatures);
    for (const sig of signatures)
        addRepeatSignature(pageNumber, sig);
    for (const para of paragraphs) {
        if (para.skipped || para.lineMarker === 'horizontal-rule')
            continue;
        const topBodyLike = para.y < viewportHeight * 0.14 &&
...
function isValidLayoutHints(data) {
    if (!data || typeof data !== 'object')
        return false;
    const x = data;
    if (x.columnsCount !== undefined && typeof x.columnsCount !== 'number')
        return false;
    if (x.sidebarGutterX !== undefined && typeof x.sidebarGutterX !== 'number')
        return false;
    if (x.gutters !== undefined && !Array.isArray(x.gutters))
        return false;
    return true;
}
async function getLayoutHints(richContent, viewport) {
    if (!layoutConfig.useModel)
        return null;
...
function toLayoutHints(data, pageWidth) {
    if (isValidLayoutHints(data))
        return data;
...
function extractLayoutNodes(data) {
...
function inferColumnsFromBoxes(boxes) {
    ...
    if (k > 1 && improvement < 0.18)
        k = 1;
    return Math.min(2, k);
}
function inferGuttersFromBoxes(boxes, columnsCount, pageWidth) {
    if (columnsCount <= 1)
        return [];
    const centers = boxes.map(b => (b.x1 + b.x2) / 2).filter(Number.isFinite);
    if (centers.length < 6)
    ...
    for (let i = 0; i < sortedCenters.length - 1; i++) {
        ...
        if (g > pageWidth * 0.12 && g < pageWidth * 0.88)
            gutters.push(g);
    }
    return gutters;
}
function isLikelySidebar(boxes, gutterX, pageWidth) {
...
function isLayoutConfigEqual(a, b) {
...
async function extractMetaFromFirstPage() {
    ...
    try {
        ...
        if (!richContent) {
            ...
            richContent = await (0, pdfRenderer_1.getPageText)(page);
...
let activeHoverParagraphId = null;
...
function hideParagraphHoverOverlay() {
    if (!paragraphHoverOverlay)
        return;
    paragraphHoverOverlay.style.display = 'none';
}
// Highlight PDF paragraph
function highlightPdfParagraph(paragraphId) {
    clearPdfHighlight();
...
textLayer.addEventListener('mouseover', (e) => {
    ...
    if (!paragraphId || paragraphId === activeHoverParagraphId)
        return;
...
document.getElementById('btn-capture')?.addEventListener('click', () => {
    saveEntirePageAsImage().catch((err) => {
        const msg = err instanceof Error ? err.message : String(err);
...
hudPageInput.addEventListener('change', e => goToPage(parseInt(e.target.value, 10)));
...
async function goToPage(n, scrollPosition) {
    if (!pdfDoc)
        return;
...
function toNumberArray(input) {
    ...
    if (ArrayBuffer.isView(input))
        return Array.from(input).map(Number).filter(Number.isFinite);
    return [];
}
function buildRenderedTextItems(items, viewport) {
    const output = [];
...
function groupRenderedLines(items) {
    if (items.length === 0)
        return [];
...
function scoreCaptionParagraph(para, viewport) {
    if (para.skipped || !para.text.trim())
        return -999;
    if (para.width < viewport.width * 0.66)
        return -999;
    if (para.y < viewport.height * 0.10 || para.y > viewport.height * 0.72)
        return -999;
    if (para.text.length < 80)
        return -999;
    const lower = para.text.toLowerCase();
    let score = 0;
    if (/^(figure|fig\.)\s*\d/i.test(para.text))
        score += 4;
    if (lower.includes('mechanism') || lower.includes('biogenesis'))
        score += 3;
    if (lower.includes('regular splicing') || lower.includes('back splicing'))
        score += 2;
    if (/\([a-z]\)/i.test(para.text))
        score += 1;
    if (para.y < viewport.height * 0.45)
        score += 1;
...
function applyTransformPoint(x, y, m) {
    return [
        m[0] * x + m[2] * y + m[4],
        m[1] * x + m[3] * y + m[5],
    ];
...
async function saveEntirePageAsImage() {
    if (!pdfDoc)
        return;
    const page = await pdfDoc.getPage(currentPage);
...
function findTableLikeClusters(paragraphs, viewport) {
    const picked = [];
    for (let i = 0; i < paragraphs.length; i++) {
        const p = paragraphs[i];
        if (p.skipped || !p.text.trim())
            continue;
        if (p.y > viewport.height * 0.72)
            continue;
        const cleanText = p.text.replace(/<[^>]+>/g, '');
        const lower = cleanText.toLowerCase();
        const headerCue = /^table\s*\d*/i.test(cleanText) ||
            lower.includes('name and genome location') ||
            lower.includes('alias in circbase') ||
            lower.includes('gene symbol') ||
            lower.includes('reference');
        const rowCue = /\[[0-9,\-–]+\]/.test(cleanText) ||
            lower.includes('chr') ||
            lower.includes('circ');
        const wideEnough = p.width >= viewport.width * 0.42;
        if ((headerCue || rowCue) && wideEnough)
            picked.push(i);
    }
    if (picked.length === 0)
        return [];
...
function cropCanvasDataUrl(rect) {
...
function injectTableImageFallback(paragraphs, viewport) {
    ...
    for (const cluster of clusters) {
        ...
        const cues = paras.reduce((sum, p) => {
            ...
            const lower = cleanText.toLowerCase();
            let s = 0;
            if (/^table\s*\d*/i.test(cleanText))
                s += 4;
            if (lower.includes('alias in circbase') || lower.includes('gene symbol'))
                s += 3;
    ...
    if (rect.width < viewport.width * 0.45 || rect.height < viewport.height * 0.08)
        return;
...
window.addEventListener('message', event => {
    ...
    switch (message.type) {
            ...
            (async () => {
                ...
                try {
                    for (const pageNum of targetPages) {
                        ...
                        const { paragraphs: pageParagraphs } = (0, textLayer_1.buildTextLayer)(dummyDiv, items, pageViewport, {
                            pageNumber: pageNum
                        });
        ...
        case 'find-and-jump-to-caption': {
            (async () => {
                if (!pdfDoc || totalPages === 0 || !message.query)
                    return;
                ...
                const getPageParagraphs = async (pageNum) => {
                    let items = pageRichContentCache.get(pageNum);
                    if (!items) {
                        try {
                            const page = await pdfDoc.getPage(pageNum);
                    ...
                    const dummyDiv = document.createElement('div');
                    const pageViewport = await pdfDoc.getPage(pageNum).then(p => p.getViewport({ scale: 1.0 }));
                    const { paragraphs } = (0, textLayer_1.buildTextLayer)(dummyDiv, items, pageViewport, {
                        pageNumber: pageNum
                    });
                    return paragraphs;
                };
...
function renderOutlineNode(items, container) {
...
async function startBackgroundScan() {
    ...
    for (let pageNum = endPage; pageNum >= startPage; pageNum--) {
        if (pageRichContentCache.has(pageNum))
            continue;
        try {
            const page = await pdfDoc.getPage(pageNum);
            const items = await (0, pdfRenderer_1.getPageText)(page);
            pageRichContentCache.set(pageNum, items);
