...
// ── Main entry ──
function buildTextLayer(container, richContent, viewport, options) {
    container.innerHTML = '';
    let styles = {};
    let commonObjs = null;
    ...
    for (const item of items) {
        if (!item.str || !item.str.trim() || !item.fontName)
            continue;
        if (isRotated(item.transform))
            continue;
        const tx = pdfjsLib.Util.transform(viewport.transform, item.transform);
        if (!tx || isNaN(tx[4]) || isNaN(tx[5]))
            continue;
        if (tx[5] < headerY || tx[5] > footerY)
            continue;
    ...
    for (const item of items) {
        ...
        if ((options?.pageNumber === 1) && isAffiliationClutter(item.str, scaledH))
            continue;
        const w = item.width * viewport.scale;
        if (isRightEdgeWatermarkToken(item.str, tx[4], tx[5], w, scaledH, viewport.width, rightEdgeNoiseZones))
            continue;
        allItems.push({
            str: item.str.trim(),
            x: tx[4],
    ...
    for (const item of allItems) {
        if (currentY === null || Math.abs(item.y - currentY) <= LINE_THRESHOLD) {
            ...
            if (currentLine.length)
                rawLines.push(currentLine.sort((a, b) => a.x - b.x));
            currentLine = [item];
            currentY = item.y;
        }
    }
        ...
        if (lineY >= firstBodyY - 5 && effectiveGutters.length > 0 && (!hasSidebar || rawLineWidth < pageWidth * 0.72)) {
            ...
            for (let j = 0; j < line.length - 1; j++) {
                ...
                for (const gutterX of effectiveGutters) {
                    const crossesGutter = rightSide1 <= gutterX + 3 &&
                        leftSide2 >= gutterX - 3;
    ...
    const orderedSegments = orderSegmentsWithHorizontalBarriers(segments, effectiveRules, hasSidebar, detectedColumnsCount);
    ...
    if (hasStructure && paragraphBoundaries && paragraphBoundaries.length > 0) {
        ...
        for (const block of pageBlocks) {
            ...
            for (let i = 0; i < rawLines.length - 1; i++) {
                ...
                if (curLine.length === 0 || nxtLine.length === 0)
                    continue;
            ...
            if (block.type === 'structured') {
                ...
                if (allText) {
                    ...
                    assignLayoutToParagraph(para, pageWidth, midX, isDoubleColumn, expandedZones, hasSidebar, sidebarGutterX, effectiveGutters, firstBodyY);
    ...
    for (const p of topParas) {
        ...
        const isHeaderOrMetadataCandidate = lowerText.includes('sciencedirect') ||
    ...
    for (const p of mergedLogicalParas) {
        ...
        if (avgY < firstBodyY - 5) {
            if (p === titlePara) {
                ...
                const lowerText = text.toLowerCase();
                const looksLongBodyProse = text.length > 110 &&
                    words.length >= 18 &&
                    !/^https?:\/\//i.test(text) &&
                ...
                const isMetadata = lowerText.includes('downloaded') ||
                    lowerText.includes('http') ||
                ...
                const isAffiliationOrFootnote = lowerText.includes('university') ||
    ...
    if (blocks) {
        console.log('[ChunZen] Blocks:', blocks.map((b) => `${b.type}(${b.segments.length}segs)`));
    }
...
function isHeadingSegment(seg) {
    ...
    // 用字符数加权，避免少量标点干扰
    const boldChars = seg.items.reduce((sum, it) => (isBoldFont(it.fontName) ? sum + it.str.length : sum), 0);
...
function isFigureCaptionSegment(seg) {
    const text = seg.str.replace(/<[^>]+>/g, '').trim();
    return /^(Figure|Fig\.?|Table)\s+\d/i.test(text);
}
function isSidebarTocHeading(seg, columnMargins, pageWidth) {
    if (seg.columnIndex < 0)
        return false;
    const text = seg.str.replace(/<[^>]+>/g, '').trim();
    if (!/^(sections?|contents?)$/i.test(text))
        return false;
    const colWidth = columnMargins.get(seg.columnIndex)?.width ?? pageWidth;
    return colWidth < pageWidth * 0.5;
}
function isSidebarTocEntry(seg, tocColumnIndex, columnMargins, pageWidth) {
    if (seg.columnIndex !== tocColumnIndex)
        return false;
    const text = seg.str.replace(/<[^>]+>/g, '').trim();
    if (!text)
        return false;
    if (/^(figure|fig\.?|table)\s+\d/i.test(text))
        return false;
    const colWidth = columnMargins.get(seg.columnIndex)?.width ?? pageWidth;
    if (seg.width > colWidth * 0.96)
        return false;
    if (text.length > 120)
        return false;
    return true;
}
function isReferenceStart(str, inReferencesSection = false, isHanging) {
    ...
    if (dotNumMatch) {
        ...
        if (!isYear) {
            if (inReferencesSection || text.length > 10)
                return true;
        }
    }
...
function isTableSegmentRow(segments, i, colWidth) {
    ...
    if (seg.width > colWidth * 0.65)
        return false;
    if (seg1.width > colWidth * 0.65)
        return false;
    if (seg2.width > colWidth * 0.65)
        return false;
    // Tight Y gaps
    const gap1 = Math.abs(seg1.y - seg.y);
    const gap2 = Math.abs(seg2.y - seg1.y);
    const lineH = Math.max(seg.height, 1);
    if (gap1 > lineH * 2.0 || gap2 > lineH * 2.0)
        return false;
...
function countWideIntraLineGaps(seg) {
...
function isLikelyTableLine(seg, colWidth) {
    if (!seg.str.trim())
        return false;
    if (seg.width > colWidth * 0.92)
        return false;
...
function detectStructuralBlocks(segments, columnMargins, pageWidth) {
    const blocks = [];
    let i = 0;
    ...
    while (i < segments.length) {
        ...
        if (pastReferencesHeading) {
            ...
            while (i < segments.length) {
                ...
                if (refSegs.length > 0 && cur.flowBand !== refSegs[refSegs.length - 1].flowBand)
                    break;
                if (isHeadingSegment(cur))
                    break;
                if (isFigureCaptionSegment(cur))
                    break;
        ...
        if (isFigureCaptionSegment(seg)) {
            ...
            while (i < segments.length) {
                ...
                if (!next.str.trim() || isHeadingSegment(next) || isFigureCaptionSegment(next))
                    break;
                if (next.flowBand !== seg.flowBand)
                    break;
                if (next.columnIndex !== seg.columnIndex && next.columnIndex >= 0)
                    break;
        ...
        if (isSidebarTocHeading(seg, columnMargins, pageWidth)) {
            ...
            while (i < segments.length) {
                ...
                if (next.flowBand !== seg.flowBand)
                    break;
                if (!isSidebarTocEntry(next, seg.columnIndex, columnMargins, pageWidth))
                    break;
        ...
        const looksLikeDenseRows = !!next1 &&
        ...
        const looksLikeDenseRows3 = !!next2 &&
        ...
        if (isTableSegmentRow(segments, i, colWidth) || (looksLikeDenseRows && looksLikeDenseRows3)) {
            ...
            while (i < segments.length) {
                ...
                if (!cur.str.trim())
                    break;
                if (cur.flowBand !== seg.flowBand)
                    break;
                if (cur.columnIndex !== seg.columnIndex)
                    break;
            ...
            while (i < segments.length) {
                ...
                if (next.flowBand !== seg.flowBand)
                    break;
                if (next.width > colWidth * 0.7)
                    break;
                if (isHeadingSegment(next) || isFigureCaptionSegment(next))
                    break;
                // Check Y gap - must be close to previous table row
                const prev = tableSegs[tableSegs.length - 1];
                if (Math.abs(next.y - prev.y) > Math.max(prev.height, 1) * 2.5)
                    break;
                tableSegs.push(next);
                i++;
            }
        ...
        while (i < segments.length) {
            ...
            if (next.flowBand !== seg.flowBand)
                break;
            if (isHeadingSegment(next))
                break;
            if (isFigureCaptionSegment(next))
                break;
            if (isSidebarTocHeading(next, columnMargins, pageWidth))
                break;
            const nextColWidth = columnMargins.get(next.columnIndex)?.width ?? pageWidth;
            if (isTableSegmentRow(segments, i, nextColWidth))
                break;
            if (isHeadingSegment(next) && /^(references|bibliography)/i.test(next.str.trim()))
                break;
            bodySegs.push(next);
            i++;
        }
    ...
    if (!blocks.some(b => b.type === 'reference')) {
        ...
        for (const seg of segments) {
            ...
            if (isReferenceStart(seg.str, true, isHanging))
                refStartCount++;
        }
...
function isContinuationOfReference(cur, prev, columnMargins) {
    let isHanging = undefined;
    if (columnMargins) {
        const colLeft = columnMargins.get(cur.columnIndex)?.left ?? 0;
        if (colLeft > 0) {
    ...
    if (isReferenceStart(cur.str, false, isHanging))
        return true;
    if (cur.flowBand !== prev.flowBand)
        return false;
    if (cur.columnIndex !== prev.columnIndex)
        return false;
...
function segmentBlockIntoParas(block, columnMargins, pageWidth, paraIdStart) {
    const paras = [];
    let paraId = paraIdStart;
    const flushPara = (text, items, section, columnIndex) => {
        if (!text.trim())
            return;
        paras.push({
            id: `p-${paraId++}`,
...
function splitTableCellsWithGeometry(seg) {
    if (seg.items.length === 0)
        return [];
...
function clusterColumnAnchors(xs, clusterDistance) {
    if (xs.length === 0)
        return [];
...
function inferTableColumnAnchors(rows, colWidth) {
...
function mapRowCellsToAnchors(cells, anchors) {
    if (anchors.length === 0)
...
function mergeContinuationTableRows(rows, anchors, colWidth) {
    if (rows.length === 0)
        return [];
    ...
    for (const row of rows) {
        ...
        while (mapped.length < expectedCols)
            mapped.push('');
...
function isBoldFont(fontName) {
    ...
    if (/bold|heavy|black|demi|extrabold|semibold/i.test(fontName))
        return true;
    // 方式二：基于频率推断（保守版）
...
function createSegment(lineItems, colIndex, styles, commonObjs) {
...
function sectionFromColumnIndex(columnIndex) {
    if (columnIndex === 0)
        return 'left';
    if (columnIndex === 1)
        return 'right';
...
function normalizeHorizontalRuleBarriers(rules, viewportWidth, viewportHeight, firstBodyY) {
    ...
    const filtered = rules
        ...
        if (width < viewportWidth * 0.25)
            return false;
        if (rule.y < viewportHeight * 0.04)
            return false;
        if (rule.y < firstBodyY - 12)
            return false;
        if (rule.y > viewportHeight * 0.98)
            return false;
        return true;
    })
        .sort((a, b) => a.y - b.y);
...
function orderSegmentsWithHorizontalBarriers(segments, rules, hasSidebar, detectedColumnsCount) {
    if (segments.length === 0)
        return [];
    if (rules.length === 0) {
        const orderedSingleBand = orderSegmentBand(segments, hasSidebar, detectedColumnsCount);
        for (const seg of orderedSingleBand)
            seg.flowBand = 0;
...
function orderSegmentBand(segments, hasSidebar, detectedColumnsCount) {
    if (hasSidebar) {
    ...
    for (let bIdx = 0; bIdx < blocks.length; bIdx++) {
        ...
        if (block.type === 'single') {
            ...
            for (const seg of block.segments) {
                ...
                if (!perColumn.has(normalizedIndex))
                    perColumn.set(normalizedIndex, []);
...
function isItalic(fontName, styles, commonObjs) {
    if (!fontName)
        return false;
...
function composeFormattedLineText(items, styles, commonObjs) {
    if (items.length === 0)
        return '';
    if (items.length === 1) {
        const it = items[0];
    ...
    for (let i = 1; i < items.length; i++) {
        ...
        const attachNoSpace = gap <= gapThreshold ||
            /^[,.;:!?%)\]}]/.test(curText) ||
            /[(\[{]$/.test(prevText) ||
            (curIsSup && !prevIsSup) ||
            (curIsSub && !prevIsSub) ||
...
function composeLineText(items) {
    if (items.length === 0)
        return '';
    if (items.length === 1)
...
function computeGutterSupport(lines, gutterX, minGapWidth) {
    let support = 0;
...
function computeColumnEvidenceByLines(lines, gutterX, pageWidth) {
    if (lines.length === 0)
        return 0;
    let evidenceLines = 0;
    let considered = 0;
...
function detectStableGutters(lines, minX, pageWidth, minGapWidth) {
    const gutterPoints = [];
    for (const line of lines) {
        if (line.length < 2)
            continue;
        for (let i = 0; i < line.length - 1; i++) {
            ...
            if (mid < minX + pageWidth * 0.12 || mid > minX + pageWidth * 0.88)
                continue;
...
function isLikelyHeadingLikeLine(text) {
    const t = text.trim();
    if (!t)
        return false;
    if (t.length > 100)
...
function mergeOverSplitBodyParagraphs(paras) {
    const merged = [];
...
function canMergeBodyParagraph(prev, cur) {
    if ((prev.blockType ?? 'body') !== 'body')
        return false;
    if ((cur.blockType ?? 'body') !== 'body')
        return false;
    if (prev.section !== cur.section)
        return false;
    if ((prev.columnIndex ?? -1) !== (cur.columnIndex ?? -1))
        return false;
    if (!prev.text.trim() || !cur.text.trim())
        return false;
    if (isLikelyHeadingLikeLine(cur.text))
        return false;
    ...
    const avgLineH = Math.max(1, (prev.items.reduce((s, it) => s + it.height, 0) + cur.items.reduce((s, it) => s + it.height, 0))
    ...
    if (yGap > Math.max(avgLineH * 1.55, 18))
        return false;
    ...
    if (prevEndsHyphen)
        return true;
    if (!prevEndsTerminal && (curStartsLower || curStartsBracket))
        return true;
    if (!prevEndsTerminal && prevShort && curShort)
        return true;
    if (prevEndsTerminal && curStartsUpper)
        return false;
...
function estimateColumnIndex(lineItems, gutters, pageWidth) {
    if (gutters.length === 0 || lineItems.length === 0)
        return -1;
...
function computeLineGapStats(segs) {
    const byColumn = new Map();
    for (let i = 1; i < segs.length; i++) {
        const prev = segs[i - 1];
        const cur = segs[i];
        if (cur.columnIndex !== prev.columnIndex || cur.section !== prev.section)
            continue;
        const gap = cur.y - prev.y;
        if (gap <= 0)
            continue;
        if (!byColumn.has(cur.columnIndex))
            byColumn.set(cur.columnIndex, []);
...
function shouldStartNewBodyParagraph(prev, cur, columnMargins, pageWidth, lineGapStats) {
    ...
    const hasFirstLineIndent = curIndent > Math.max(cur.height * 1.6, 12) &&
    ...
    if (curStartsList)
        return true;
    if (hasFirstLineIndent && !curStartsLower && prevEndsTerminal && spacingBreak)
        return true;
    return false;
}
function getPlainTextIndexToHtmlIndexMap(html) {
    const map = [];
    let inTag = false;
...
function computeItemOffsets(text, items) {
...
function splitIntoSentences(text) {
    ...
    if (last < protectedText.length)
        results.push(protectedText.slice(last));
    return results
...
function detectRightEdgeNoiseZones(items, viewport) {
    const candidates = [];
    for (const item of items) {
        if (!item.str || !item.str.trim())
            continue;
    ...
    if (candidates.length === 0 || !candidates.some(c => c.anchor))
        return [];
...
function isRightEdgeWatermarkToken(text, x, y, width, height, viewportWidth, rightEdgeNoiseZones) {
    const t = text.trim().toLowerCase();
    if (!t)
        return false;
    const looksWatermarkLexeme = t === 'downloaded' ||
        t === 'from' ||
        /^https?:\/\//.test(t) ||
    ...
    if (inNoiseZone && height <= 8.5 && (looksWatermarkLexeme || weakLexeme))
        return true;
    if (!looksWatermarkLexeme)
        return false;
    const rightEdge = x >= viewportWidth * 0.92 || (x + width) >= viewportWidth * 0.985;
    const tinyFont = height <= 7.5;
    return rightEdge && tinyFont;
}
function inferFirstBodyY(lines, viewportHeight, pageNumber) {
    if (lines.length === 0)
    ...
    for (const line of lines) {
        ...
        if (!text)
            continue;
        const lower = text.toLowerCase();
        if (isFirstPage && isLikelyAuthorList(text))
            continue;
        if (lower.includes('corresponding author') ||
 ...
 */
function isAffiliationClutter(str, scaledHeightPx) {
...
function isRunningHeaderFooter(lineStr, lineY, viewportHeight) {
    ...
    if (lower.includes('copyright') || lower.includes('©') || lower.includes('all rights reserved'))
        return true;
    if (lower.includes('doi:') || lower.includes('doi.org'))
        return true;
    if (lower.includes('issn') || lower.includes('e-issn') || lower.includes('eissn'))
        return true;
    ...
    if (/\bvol\.\s*\d+/i.test(trimmed) || /\bno\.\s*\d+/i.test(trimmed))
        return true;
    if (/\b(?:january|february|march|april|may|june|july|august|september|october|november|december)\s+\d{4}\b/i.test(trimmed))
        return true;
...
function mapRoleToBlockType(role) {
    if (!role)
        return 'unknown';
    if (role === 'p')
        return 'body';
...
function assignLayoutToParagraph(para, pageWidth, midX, isDoubleColumn, expandedZones, hasSidebar, sidebarGutterX, effectiveGutters, firstBodyY) {
...
function shouldSkipParagraphForTranslation(para, viewportWidth, viewportHeight, firstBodyY, hasSidebar) {
    ...
    const nearTopBodyProse = avgY <= firstBodyY + 28 &&
        text.length > 110 &&
        wordCount >= 18 &&
        !startsWithHttp &&
        !lowerText.includes('downloaded from');
    if (nearTopBodyProse)
        return null;
    // 1. Sidebar / TOC
    if (hasSidebar && para.columnIndex === 1)
        return 'sidebar-column';
    // 2. Authors/Headers blockType
    if (para.blockType === 'authors' || para.blockType === 'header')
        return 'header-or-authors';
    // 3. Running Headers / Footers metadata
    if (isExtremeTop || isExtremeBottom) {
        if (lowerText.includes('copyright') || lowerText.includes('©') || lowerText.includes('all rights reserved'))
            return 'edge-metadata';
        if (lowerText.includes('doi:') || lowerText.includes('doi.org'))
            return 'edge-metadata';
        if (lowerText.includes('issn') || lowerText.includes('e-issn') || lowerText.includes('eissn'))
            return 'edge-metadata';
        ...
        if (/^\d+\s+of\s+\d+$/i.test(text))
            return 'edge-metadata';
        if (lowerText.includes('downloaded from') || lowerText.includes('published by') || lowerText.includes('sciencedirect'))
            return 'edge-metadata';
        if (lowerText.includes('consensus statement') || lowerText.includes('check for updates'))
            return 'edge-metadata';
    }
    ...
    if (isMetadata && avgY < firstBodyY + 100)
        return 'top-metadata';
    ...
    if (lowerText === 'downloaded' || lowerText === 'from')
        return 'watermark-fragment';
    // Treat right-side http links as page-side metadata/noise.
    if (startsWithHttp && rightSideLike)
        return 'right-http-noise';
...
function isLikelyAuthorList(text) {
