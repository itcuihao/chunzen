...
function classifyTag(tag) {
    ...
    if (HEADING_TAGS.has(t))
        return 'heading';
    if (PARAGRAPH_TAGS.has(t))
        return 'p';
    if (FIGURE_TAGS.has(t))
        return 'figure';
    if (TABLE_TAGS.has(t))
        return 'table';
    return 'other';
...
async function getPageText(page, viewport) {
    ...
    console.log(`[ChunZen] getPageText: items=${items.length}`, `hasStructure=${hasStructure}`, `boundaries=${dedupedBoundaries.length}`, `horizontalRules=${horizontalRules.length}`);
    return {
        items,
...
function dedupBoundaries(boundaries) {
...
function extractPathLinesFromConstructPathArgs(args, ctm, viewportTransform) {
    ...
    for (const op of ops) {
        ...
        if (lineToCode !== undefined && op === lineToCode) {
            if (i + 1 >= data.length || !hasCurrentPoint)
                break;
            const nx = data[i++];
            const ny = data[i++];
...
function isStrokeLikeOp(op, OPS) {
    const strokeLike = [
        OPS.stroke,
        OPS.closeStroke,
        OPS.fillStroke,
...
async function extractHorizontalRules(page, viewport) {
    ...
    const flushPath = (strokeLike) => {
        ...
        for (const ln of pendingLines) {
            ...
            if (length < localViewport.width * 0.18)
                continue;
            const dy = Math.abs(ln.y2 - ln.y1);
            if (dy > Math.max(2.2, lineWidth * localViewport.scale * 1.3))
                continue;
            lineCandidates.push({
        ...
        for (const box of pendingBoxes) {
            ...
            if (width < localViewport.width * 0.25)
                continue;
            if (height > Math.max(localViewport.height * 0.02, 8))
                continue;
            boxCandidates.push({
                x1: box.minX,
                x2: box.maxX,
    ...
    for (let i = 0; i < operatorList.fnArray.length; i++) {
        ...
        const consumesPath = (endPathCode !== undefined && fn === endPathCode) ||
    ...
    const inBounds = candidates.filter(c => {
        if (!Number.isFinite(c.x1) || !Number.isFinite(c.x2) || !Number.isFinite(c.y))
            return false;
        if (c.y < 0 || c.y > localViewport.height)
            return false;
