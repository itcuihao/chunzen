...
class OpenAIEngine {
    ...
    async translate(text, sourceLang, targetLang, glossary) {
        const cfg = vscode.workspace.getConfiguration('chunzen.translation.openai');
        const apiKey = cfg.get('apiKey', '').trim();
        const baseUrl = cfg.get('baseUrl', 'https://api.openai.com/v1').trim();
        const model = cfg.get('model', 'gpt-4o-mini').trim();
...
class CustomHttpEngine {
    ...
    async translate(text, sourceLang, targetLang, glossary) {
        ...
        const responsePath = cfg.get('responsePath', 'result').trim();
        if (!url) {
