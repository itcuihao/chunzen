...
class MineruService {
    async slicePdf(pdfPath, maxPages) {
        try {
            ...
            const subPdfDoc = await pdf_lib_1.PDFDocument.create();
     ...
     */
    async parsePdf(pdfPath, config, onProgress) {
    ...
    async parseWithAgent(pdfPath, isUrl, fileName, onProgress) {
        ...
        try {
            ...
            if (isUrl) {
                ...
                for (let attempt = 1; attempt <= maxRetries; attempt++) {
                    try {
                        response = await (0, fetch_1.customFetch)('https://mineru.net/api/v1/agent/parse/url', {
                            method: 'POST',
                ...
                for (let attempt = 1; attempt <= maxRetries; attempt++) {
                    try {
                        response = await (0, fetch_1.customFetch)('https://mineru.net/api/v1/agent/parse/file', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({
                ...
                for (let attempt = 1; attempt <= maxRetries; attempt++) {
                    try {
                        uploadResponse = await (0, fetch_1.customFetch)(uploadUrl, {
                            method: 'PUT',
                            body: fileBuffer
                        });
    ...
    async pollAgentTask(taskId, onProgress) {
        const maxRetries = 120; // Allow polling up to ~6 minutes
        const intervalMs = 3000;
        onProgress('parsing', 30, '云端解析排队中...');
        for (let i = 0; i < maxRetries; i++) {
            await new Promise(resolve => setTimeout(resolve, intervalMs));
            const response = await (0, fetch_1.customFetch)(`https://mineru.net/api/v1/agent/parse/${taskId}`);
            if (!response.ok)
                continue;
            const res = (await response.json());
            if (res.code !== 0) {
            ...
            if (state === 'done') {
                ...
                let mdResponse;
                try {
                    mdResponse = await (0, fetch_1.customFetch)(mdUrl);
                }
                catch (fetchErr) {
    ...
    async parseWithStandard(pdfPath, isUrl, fileName, token, onProgress) {
        onProgress('parsing', 5, '正在提交 Standard 解析任务...');
        let taskId = '';
        let batchId = '';
        if (isUrl) {
            const response = await (0, fetch_1.customFetch)('https://mineru.net/api/v4/extract/task', {
                method: 'POST',
                headers: {
            ...
            const response = await (0, fetch_1.customFetch)('https://mineru.net/api/v4/file-urls/batch', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
            ...
            const uploadResponse = await (0, fetch_1.customFetch)(uploadUrl, {
                method: 'PUT',
                body: fileBuffer
            });
            if (!uploadResponse.ok) {
    ...
    async pollStandardTask(taskId, token, onProgress) {
        const maxRetries = 120;
        const intervalMs = 3000;
        onProgress('parsing', 30, '云端 Standard 解析排队中...');
        for (let i = 0; i < maxRetries; i++) {
            await new Promise(resolve => setTimeout(resolve, intervalMs));
            const response = await (0, fetch_1.customFetch)(`https://mineru.net/api/v4/extract/task/${taskId}`, {
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            });
    ...
    async pollStandardBatch(batchId, token, onProgress) {
        const maxRetries = 120;
        const intervalMs = 3000;
        onProgress('parsing', 30, '云端 Standard 批处理排队中...');
        for (let i = 0; i < maxRetries; i++) {
            await new Promise(resolve => setTimeout(resolve, intervalMs));
            const response = await (0, fetch_1.customFetch)(`https://mineru.net/api/v4/extract-results/batch/${batchId}`, {
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            });
    ...
    async downloadAndUnzip(zipUrl) {
        const tempDir = os.tmpdir();
        const tempZipPath = path.join(tempDir, `mineru_${Date.now()}.zip`);
        try {
            let response;
            try {
                response = await (0, fetch_1.customFetch)(zipUrl);
            }
            catch (fetchErr) {
            ...
            return new Promise((resolve, reject) => {
                ...
                (0, child_process_1.exec)(`unzip -p "${tempZipPath}" "*full.md"`, { maxBuffer: 1024 * 1024 * 25 }, (error, stdout, stderr) => {
                    if (error) {
...
//# sourceMappingURL=mineruService.js.map